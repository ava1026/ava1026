...
> 性质：竞品情报，内部使用，非对外材料
> 信息标注约定：**〔一手〕** = GMGN 内部实测/抓包/影子测试；**〔公开〕** = 外部公开报道或官方口径；**〔推断〕** = 基于以上两类数据的推理，未经验证；**〔X 抓取〕** = X 公开数据的二手整理，未经核验

---

## 📐 这份文档在哪一层

FOMO 情报分三层，各司其职，**同一个事实只在一处定义**：

| 层 | 文档 | 回答什么 | 什么时候看 |
|---|---|---|---|
| **Tier 0** | [FOMO 决策简报](https://claude.ai/code/artifact/76b7997d-e74f-4e77-bb3e-66d986048bef) | 结论 · 威胁分级 · P0/P1 | 转发给决策层，一屏读完 |
| **Tier 1** | [FOMO 增长闭环](https://claude.ai/code/artifact/64861aa8-fe7c-4912-b01a-1ee52aae0087) | **病在哪** —— 飞轮机制、铰链、漏损点、获客导图 | 想理解它为什么转得快 |
| **Tier 2** | **本文档** | **怎么治 + 全部证据** —— 资本、数据、技术拆解、费率、口碑、行动清单、附录 | 查具体数字、地址、出处 |

所有跨文档复用的数字收敛在 `docs/_build/fomo_facts.py`；改数字只改那里，然后跑 `python3 docs/_build/check-facts.py` 校验本文档是否漂移。
...
| **新链支持慢** | 需要等第三方（Relay 等）覆盖后才能上，自己无法自研 | 中——见下方注 |
| **"限价单"非链上真单** | 创始人公开承认非托管跨 5 链做真限价单"极难"，只能先做 SOL | 中——可作为对比素材 |
| **技术全外包** | 路由、跨链、密钥、入金、出金全靠第三方 | 长期——供应商风险，但短期是它的速度优势 |
| **本地化几乎为零** | 仅英文 | 高——新兴市场空白 |

> **⚠️ 「新链支持慢」这条与我们自己的另一条实测相矛盾，需要裁决。**
> 结构上它确实**受制于第三方**——能上哪条链取决于 Relay 等供应商是否覆盖，自己无法自研。
> 但实际速度并不慢：**Robinhood Chain 2026-07-01 主网上线，创始人 07-05 对「支持 RH 链」只回了一个 👀，我们 07-13 就抓到了它在 RH 链上的完整跨链交易 —— 从表态到落地 ≤ 8 天**（见 §4.1、§12.1 的 RH 链地址索引）。
> 所以准确的表述是「**新链支持受制于第三方的覆盖范围**」（能力天花板），而不是「因此慢」（速度问题）。**供应商支持了它就很快。**
> 若要在对外材料里用「慢」这个说法，先解决这个矛盾——否则我们自己的报告里同时写着「8 天上新链」和「上新链慢」。
...
> ### 📏 统一口径：先分清「哪条链、哪个指标」再看排名
>
> 本节引用的全部是**公开报道的 Solana 单链交易量与协议收入**。这与**全链交易量**不是一回事，两者并排会造成误读。
>
> **同窗口同链重算后的全链交易量**〔Arthur v5，2026-08-13〕：
>
> | 窗口 | GMGN | FOMO | Axiom |
> |---|---|---|---|
> | 30 天 | **$2.6B** | $1.7B | $1.6B |
> | 24 小时 | **$89M** | $71M | $69M |
>
> **两个窗口 GMGN 均为第一。**此前流传的「Axiom 第一 / 我们与 FOMO 差距不到 2%」来自 DefiLlama 漏链的旧数——**该口径已作废**。
>
> **⚠️ 同一个根因，两边都中招**：DefiLlama 的链覆盖不完整——
> - 漏掉 **FOMO 的跨链收入**（平台费在 Relay 报价内含扣除，链上无独立转账，见 §3.2 口径陷阱）
> - 漏掉 **GMGN 的 Robinhood 链**（把我们算低了）
>
> 所以「98.5% 收入来自 Solana」和「Axiom 交易量第一」**是同一个测量缺陷的两个侧面**。引用任何 DefiLlama 排名前，先确认它覆盖了哪几条链。
>
> **对结论的影响**：威胁**不在当前量级**，在**增长斜率与渠道网络**。本报告的紧迫性论证应按这个前提读——我们不是在追赶，是在守住第一的同时补社交与破圈这条腿。
...
| 交易额 | 计费方式 | 实际费率区间 | 实测样本 |
|---|---|---|---|
| ≤ 200 USDC | **固定 0.95 USDC/笔** | **0.48% – 47.5%**（随金额递减） | 2.00U 收 0.95（47.5%）· 184.64U 收 0.95（0.51%）|
| 200 ~ 10,000 USDC | **0.5% 比例** | 0.50% | 226.67U 收 1.133362（0.50%）|
| ≥ 10,000 USDC | **0.05% 比例** | 0.05% | — |

> 固定档的实际费率取决于交易额，**单看某一端都不代表这一档**。区间上界（47.5%）出现在 $2，下界（0.48%）出现在 200U。

> **正确的读法是「固定费结构」，不是某个点位的百分比。**≤200U 收固定 0.95 USDC/笔，意味着**实际费率随交易额一路衰减**：$2 时 47.5%、$50 时 1.9%、$95 时 1.0%、$200 时 0.48%。
> 拿"$2 交易收 47.5%"当结论是选择性取样——那是这个结构的最极端点，不代表这一档的普遍体验。它确实解释了 App Store 上"只有你有 1 万美元以上才好用"这类负评的来源，但**不构成一个可用于对外传播的论点**（理由见下方 §6.5 与 §10）。

### 6.5 与 GMGN 的费率对照〔一手 × 内部三源〕

**GMGN 侧口径：1% 比例费率**，三处内部资料一致：
- 《0427 运营活动方案》：每交易 $10 收 1% = $0.10（用于单位经济测算）
- 《竞品社群更新》：官方对用户回应「平台仅收 1% 手续费，其余以链上为准」
- 《App Store 投放策略》：中位月交易额 $609–630 → 每用户月产生 ~$6 手续费，反算 ≈0.98%

⚠️ 是否全链 / 全产品线一致、有无 VIP 或折扣档，**待确认**（见 §11 第 16 项）。

| 交易额 | FOMO 收费 | FOMO 实际费率 | GMGN 1% | 谁便宜 |
|---:|---:|---:|---:|---|
| $2 | 0.95（固定） | 47.5% | $0.02 | GMGN 47.5× |
| $20 | 0.95（固定） | 4.75% | $0.20 | GMGN 4.7× |
| $50 | 0.95（固定） | 1.90% | $0.50 | GMGN 1.9× |
| **$95** | 0.95（固定） | **1.00%** | $0.95 | **持平（临界点）** |
| $200 | 0.95（固定） | 0.48% | $2.00 | FOMO 2.1× |
| $1,000 | 0.5% | 0.50% | $10.00 | FOMO 2.0× |
| $9,999 | 0.5% | 0.50% | $99.99 | FOMO 2.0× |
| $10,000 | 0.05% | 0.05% | $100.00 | **FOMO 20×** |
| $100,000 | 0.05% | 0.05% | $1,000.00 | **FOMO 20×** |

**结论（对我们不利）**：
1. **我们的价格优势只存在于 $95 以下**，且随金额迅速收窄，到 $200 就已反转。
2. **$200–$10,000 区间 FOMO 恒定便宜一半**（0.5% vs 1%）——这是零售交易的主力区间。
3. **$10,000 以上 FOMO 便宜 20 倍**。档位在 $10,000 处有断崖：$9,999 收 $50，$10,000 只收 $5。
4. 这为 2025-11 KOL 反馈的「**很多巨鲸交易中高市值代币都跑到 FOMO 上面去了**」提供了一个机械解释——大额段省 95% 手续费。此前这两个观察是分开记录的，**现在应视为可能相关**（仍需 §11 验证）。
...
> **📐 分层说明**：增长机制的**论证**（飞轮如何闭合、铰链在哪、四条加速支路、影响力发生器）已移入 **[《FOMO 增长闭环》](https://claude.ai/code/artifact/64861aa8-fe7c-4912-b01a-1ee52aae0087)** —— 那份文档回答「病在哪」。
> 本节只保留**原始名单、实测数据与画像判断**，供随机查阅；行动建议统一在 §10。
...
- **Apple Pay 入金**（Crossmint）：68,000+ 首次买币用户，$25M。涉及支付合规，追平成本高。
- **Affiliate 计划**：`fomo.family/affiliates`，实时结算、无上限。
- **X Affiliation Badge**：X Verified Organization 的关联认证，显示在 KOL **自己的主页**上（黑色方形图标 + 蓝 V）。官方未大规模统一发放，部分活跃用户持有。
- **Pump.fun 挖角战**（2026-08-09）：$20k 签约金 + $30k/月，条件含**公开转移资金、永久删除 FOMO 账号、签排他协议**。

### 7.5 X 生态账号名单〔X 抓取，二手，粉丝数为抓取时点值〕

> ⚠️ 本节数据来自 X 公开抓取的二手整理，非我们一手观测。源数据自身标注了句柄匹配的不确定性（如 @btc_goose 系 @thebtcgoose 的最佳匹配），**账号级准确性未经核验**，见 §11 第 14 项。

**A. 原生红人（官方点名，含站内放大倍数）** —— FOMO 粉丝除 @hdegrootvan 为本人自报外，均由「X 粉丝 × 官方公布倍数」推算：

| 账号 | X 粉丝 | 倍数 | FOMO 粉丝 | 备注 |
|---|---|---|---|---|
| @wwayfboss | 752 | **27×** | ≈20,304（推算） | 倍数最高 |
| @octoseaa | 2,636 | 5.1× | ≈13,444（推算） | |
| @hdegrootvan | 7,943 | 2.1→2.9× | **23,000+（自报）** | 社交情绪型代表 |
| @f4vel | 3,055 | 2.7× | ≈8,248（推算） | |
| @thokani | 8,073 | 2.0× | ≈16,146（推算） | 曾自报 12k（时点更早） |
| @icemandot | 8,091 | — | — | 钱包余额顶级，资金实力型 |

**关键统计**：X 粉丝跨度 **10.7×**（752 → 8,073），FOMO 粉丝跨度仅 **2.8×**（8,248 → 23,035）。⚠️ **存在选择偏差**：官方只会点名倍数高的样本，n=5，不代表平台平均水平。机制解读见[增长闭环 §04](https://claude.ai/code/artifact/64861aa8-fe7c-4912-b01a-1ee52aae0087)。

**B. 官方重点点名庆祝（涨粉 / Leaderboard 里程碑）**

| 账号 | X 粉丝 | 时间 | 官方互动 |
|---|---|---|---|
| @ResellCalendar | 104,213 | 2026.6 | 从零冲到 all-time top 5 |
| @PoorGoat_ | 70,516 | 2026.7–8 | 多次庆祝涨粉超 10 万 |
| @remusofmars | 35,973 | 2026.3 | 超 10 万粉丝 |
| @0xAvast | 16,737 | 2026.8 | 超 10 万粉丝 on fomo |
| @jotagezin | 12,792 | 2026.5 | 同时登顶所有 Leaderboard |
| @pingucharts | 8,605 | 2026.6 | #1 + Discord 合作教育者 |
| @Binkieeefomo | 5,807 | 2026.7 | 达到 10 万粉丝 |
| @XVentures_Fund | 2,710 | 2026.6 | 上周 Leaderboard #1 |

**C. 外部 KOL 加入（2026.7–8「投奔期」）**

| 账号 | X 粉丝 | 时间 | 类型 |
|---|---|---|---|
| @theunipcs | 231,973 | 2026.5.22 | 官方欢迎 |
| @ashrobin | 98,648 | 2026.5.30 | 正式合作（content creator） |
| @andyyy | 76,439 | 2026.8.10 | 宣布加入 |
| @mattkalish | 44,853 | 2026.8 上旬 | 加入被曝光（DraftKings 相关） |
| @0xyoshitaka | 17,088 | 2026.7 中下旬 | 宣布加入 |
| @C1phervoyager | 6,574 | 2025.8–9 | 早期加入 |
| @MakroMiloz | 1,139 | 2026.8.10 | 宣布加入 |
| @0xDakshKumar | 829 | 2026.8.9 | 宣布加入 |

**其他高互动影响力账号**：@orangie（377,423）、@EricCryptoman（267,103）、@real_dr_pump（11,089）、@loganlim_x（3,353）。

**D. 跟单机制**〔X 抓取〕：以**社交跟单**为主 —— Follow 后接收通知，**用户手动跟单买入**（系统不自动镜像执行）。源报告称后期增加「更接近自动 Copy」的功能，措辞是*更接近*，**未说明已实现全自动**；且承认「细粒度复制成交量属于 App 内部数据，外部无法完整获取」—— **跟单真实执行率外部不可见**。
...
| **费率（<$95）** | 1% 比例 | ≤200U 固定 0.95 USDC/笔 | ✅ 我们领先，但**优势随金额迅速收窄** |
...
| 威胁 | 等级 | 理由 |
|---|---|---|
| **抢占新增用户入口** | 🔴 **最高** | 法币入金 + USDC 全链交易 +（无 gas、滑点等感知），68k 首次买币用户，日增 3k+。这些用户**从未见过 GMGN**，心智一旦形成极难夺回 |
...
| **追踪缺口导致用户流失** | 🟠 中高 | **这是我们的解析 bug，不是能力差距**：Relay 代发使买单归属到 relayer，用户钱包只剩 0 金额转入 → 聪明钱转战 FOMO 后在我们这里「消失」。**数据是我们的核心卖点，这个洞恰恰打在上面**；且盲区随 FOMO 增长而扩大，Debot / basedbot 能解析而我们不能，KOL 已正式投诉。已定位，见 §9.1 |
...
> **📐 这是唯一的行动清单。**[《FOMO 增长闭环》](https://claude.ai/code/artifact/64861aa8-fe7c-4912-b01a-1ee52aae0087) 只做诊断（按闭环环节看我们的位置），不重复列行动——避免两处优先级打架。
> 需要一页纸转发给决策层的版本见 **[《FOMO 决策简报》](https://claude.ai/code/artifact/76b7997d-e74f-4e77-bb3e-66d986048bef)**。
...
6. ~~小额费率对比作为增长素材~~ —— **已撤销**。见 §6.5：我们的价格优势只在 $95 以下，$200 以上全线落后、$10k 以上落后 20 倍。做这个对比等于把注意力引到一张我们大部分区间都输的对照表上。
   → **改为**：先把 §6.5 的完整对照表同步给增长与 BD，**在对外口径中避免主动挑起费率话题**；若被用户或竞品提起，按分段如实回应。大额段费率是否要跟进，属于定价决策，需单独立项。
...
> 本节合并了原先散在三处的待验证项：情报缺口清单、[§3 口径陷阱](#三业务数据与增长曲线)的方法 A–D、[§7.5](#七增长与市场策略) X 抓取数据的四条 caveat。
> **归属列是建议职能，不是指派。** 认领人一列请自行填写——没有认领人的缺口不会有人去查。

| # | 待验证 | 为什么重要 | 建议方法 | 优先级 | 建议归属 | 认领人 |
|---|---|---|---|---|---|---|
| 1 | **各链收入拆分：跨链费是否含 Robinhood** | "98.5% 来自 Solana"是 DefiLlama 测量口径产物；跨链平台费链上无独立转账，RH / Base / BNB / Monad 真实贡献**完全未知**。直接决定要不要跟进 RH 链 | 方法 A：统计跨链桥程序 `99vQwtBwYtrqqD9YSXbdum3KBdxPAVxYTaQ3cfnJSrN2` 的 USDC 存入量 × 0.5%。**一人一天可出结果，不依赖第三方** | 🔴 **P0** | 数据侧 | |
| 2 | **「离散度塌缩」无偏采样** | 若在全量用户上成立，FOMO 的护城河比我们估计的深得多；目前 n=5 且是官方挑选的高倍数样本 | Sensor Tower / data.ai + 站内随机采样，直接观测 FOMO 粉丝数而非推算 | 🔴 **P0** | 情报侧 | |
| 3 | 真实 DAU / 留存曲线 | 625k 是注册数；唯一能算干净的是 2025-11 的 29% trader/user 单点值。**没有留存就无法判断增长质量** | 第三方 App 数据；链上活跃钱包数估算 | 🟠 P1 | 情报侧 | |
| 4 | Robinhood 链上 FOMO 实际体量 | 已知技术支持，不知用量。RH 链 7 月上线即 $305M TVL、1M 周活地址，且 memecoin 主导 | 对 RH（4663）上 `RelayRouterV3 0xb92fe925…fff4f` 的成交按链统计（方法 C） | 🟠 P1 | 数据侧 | |
| 5 | 跨链卖出侧平台费 | 补齐方法 A 的另一半，得到完整跨链收入 | 方法 B：统计 Solana Relayer `F7p3dFrjRTbtRp8FRF6qHLomXbKRBzpvBLjtQcfcgmNe` 转给用户的 USDC × 0.5%，**剔除纯 USDC 归集**（该场景不收费） | 🟠 P1 | 数据侧 | |
| 6 | Relay 的 app fee 结算口径 | 权威答案。我们本来就要启动 Relay BD，账单口径可在商务谈判中直接问到 | 方法 D：BD 直接问 | 🟠 P1 | BD | |
| 7 | 限价单 SOL 版上线时点 | 上线即抹平我们一个优势 | 监控 App 更新日志 + @seyong 的 X | 🟠 P1 | 情报侧 | |
| 8 | 自动跟单是否已真正实现 | 源报告只说"更接近自动 Copy"，未说已全自动。若已实现，说明它绕过了 Privy 签名限制，技术判断要改 | 产品实测：设置跟单后离线，看是否仍成交 | 🟡 P2 | 产品 + 情报侧 | |
| 9 | Privy recovery 的实际治理条件 | "≈全权控制"目前是技术推断；对外使用前需坐实 | Privy 公开文档 / 安全审计报告 | 🟡 P2 | 安全 / 技术 | |
| 10 | 出金（Spritz）失败率 | 它最大的软肋，量化后是最硬的攻击素材 | 舆情抓取 + 小额实测走通全流程 | 🟡 P2 | 情报侧 + 实测 | |
| 11 | FOMO 的 KOL / sponsored creator 商务条件 | 决定我们抢 KOL 的报价 | BD 侧打听 | 🟡 P2 | BD | |
| 12 | Pump.fun 挖角战果 | 检验 FOMO 用户粘性的天然实验 | 监控双方交易量与红人账号动向 | 🟡 P2 | 情报侧 | |
| 13 | 是否会发币 / 上积分 | 会大幅改变增长曲线与用户质量 | 监控官方渠道 | 🟡 P2 | 情报侧 | |
| 14 | **X 抓取名单的句柄核验** | §7.5 全部数据的地基。源数据自身标注了匹配不确定性（@btc_goose vs @thebtcgoose） | 逐个核对 X 句柄与粉丝数，标注抓取时点 | 🟡 P2 | 情报侧 | |
| 16 | **GMGN 自身费率口径确认** | §6.5 全部对照的基准。三处内部资料一致指向 1%，但是否全链 / 全产品线一致、有无 VIP 或折扣档未确认。**基准错了整张对照表都错** | 找计费/产品侧确认现行费率表 | 🔴 **P0** | 产品 + 计费 | |
| 17 | 巨鲸迁移与费率的因果 | §6.5 提出大额段省 95% 可能是巨鲸迁移的机械原因，目前只是相关性猜测 | 取一批已迁移的大额钱包，看其在 FOMO 的交易额分布是否集中在 ≥$10k 档 | 🟠 P1 | 数据侧 | |
| 15 | 团队规模与组织 | 判断它的迭代上限 | LinkedIn / 招聘信息 | 🟢 P3 | 情报侧 | |

**两条 P0 的共同点**：都不依赖第三方数据源，都能用我们已有的地址和工具做出来，且都直接影响一个正在等答案的决策。建议本周内先出这两条。
