...
<p><strong>定位不动：专业交易能力与专业交易员是主导区。</strong>App 侧重<strong>发现层</strong>——第一时间发现热门币，放大其中 KOL 与大户的真实持仓，把链上新用户拉进来。</p>

<h2>01 — 全局：五个模块怎么转起来</h2>

{table(["①发现", "②放大", "③拉新承接", "④交易"],
       [["热门币第一时间抓到并推出", "KOL 与大户的真实持仓",
         "法币入金 · USDC · 迁移引导", "专业执行 · 参数可控"]])}

<p>④ 的链上数据<strong>回流</strong>到 ①，让下一轮「什么在热、谁在赚」更准——环就是这样合上的。</p>

{panel("warning",
  "<p><strong>⑤ 地基：Relay 交易解析 · 标注与可信度规范 · 推送延迟〔待核〕· 客服口径。</strong>"
  "四条都托在上面这一圈底下，不产生用户价值，但不清掉环转不起来。</p>")}
...
<ul>
  ...
  <li><strong>拉新与承接</strong>
    <ul>
      <li>客源＝链上新用户，不是圈外小白</li>
      <li>法币入金（已支持，新链上的天然优势）</li>
      <li>热门代币支持 USDC 购买（限新用户）</li>
      <li>跨链：待调研，不进老用户主路径</li>
...
{table(["环节", "人群", "归属", "抓手"],
       [["<strong>发现</strong>", "链上新用户 · 打二段用户", "App 主轴",
...
{panel("note",
  "<p>这条回答 v8 待裁决 ①：不是否掉「分人群」，而是把<strong>分的位置</strong>定下来——"
  "<strong>在环节里分，不在端上分</strong>，所以不必二选一，也不让出主导区。</p>"
...
{table(["动作", "状态", "要点"],
       [["<strong>热门精选代币 push</strong>", OK(),
        ...
        [f"<strong>我们自己的推送延迟</strong>：{V['push_delay']}", HOLD(),
...
{panel("error",
  "<p><strong>两条红线。</strong>一、<strong>呈现为数据服务，不是打假</strong>"
  "（口径是「它的榜不区分来源」，不是「它造假」）。"
  "二、<strong>必须带来源标注与可信度提示</strong>，因为我们目前无法识别投毒，"
  "relay 交易只敢解析成转入 / 转出。</p>")}
...
<h2>05 — 拉新与承接：客源是链上新用户，不是圈外小白</h2>

{table(["动作", "状态", "要点"],
       [["<strong>法币入金</strong>", OK(),
         "新链上的天然优势：新链没有原生资产存量，用户无从入金。口径待核，见 §09"],
        ["<strong>热门代币支持 USDC 购买</strong>", Q(), "<strong>限新用户</strong>。见下"],
        ["跨链", HOLD(), "清单见下，不进老用户主路径"],
        ["<strong>迁移引导路径</strong>", NEW(),
         "一键导入私钥 / 接管持仓 / 落地页。出口是它自己开的——"
         "官方建议用户「导出私钥到 Phantom 操作」"]])}

{panel("note",
  "<p>限定在<strong>已经热门的代币</strong>是对的：热门币已毕业、流动性好、Relay 报价源覆盖得到，"
  "<strong>恰好是 relay 唯一不拖后腿的集合</strong>，也正好是新用户会买的东西。</p>"
...
{panel("note",
  "<p>只收结构性短板。费率、本地化、出金体验属<strong>窗口期</strong>——它 08 月已在降费率，"
  "<strong>别写进产品叙事</strong>；参考它从创始人一个 👀 到上线 RH 链只用了 8 天。</p>")}
...
{table(["事项", "为什么会改结论"],
       [["<strong>FOMO 的 Apple Pay 是否已移除</strong>",
        ...
        ["<strong>我们法币入金的实际口径</strong>",
         "覆盖哪些链 / 国家？一键直购还是入金到余额再买？"
         "主报告 §9 现在写的是「入金门槛 ❌ FOMO 明显领先」，改这行得先有口径"],
...
{panel("warning",
  "<p><strong>要盯：它的榜单口径会不会变。</strong>持仓页已经区分 transferred / bought，"
  "说明字段在数据结构里、只是榜没用，改的成本可能远低于我们的假设。"
  "建议加进 v8 观察哨。</p>")}
